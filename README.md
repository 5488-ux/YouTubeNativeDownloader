...
原生 SwiftUI YouTube 下载器，支持 iOS 16.1 及以上版本（含 iOS 26）。5.1 使用服务器 yt-dlp 解析，iPhone 负责媒体下载、本机合并和保存，并提供三阶段灵动岛与锁屏进度。
...
- 灵动岛展开视图和锁屏分别显示服务器解析、视频下载、AAC 下载三条独立进度，任务结束后立即移除。
- 显示下载速度、传输大小、剩余时间和后台任务。
